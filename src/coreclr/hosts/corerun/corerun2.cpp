...
struct configuration
{
    ...
    pal::string_t entry_assembly_fullpath;
...
// Class used to perform specfic actions.
...
static pal::string_t build_tpa(const pal::string_t& core_root, const pal::string_t& core_libraries)
{
    ...
    for (const char_t* const* curr_ext = tpa_extensions; *curr_ext != nullptr; ++curr_ext)
    {
        ...
        for (const pal::string_t& dir : { core_libraries, core_root })
        {
            if (dir.empty())
                continue;

            assert(dir.back() == pal::dir_delim);
            pal::string_t tmp = pal::build_file_list(dir, ext, [&](const char_t* file)
                {
                    pal::string_t file_local{ file };
...
static bool try_load_hostpolicy(pal::string_t mock_hostpolicy_value)
{
    pal::mod_t hMod;

#ifdef TARGET_WINDOWS
    const char_t* hostpolicyName = W("hostpolicy.dll");
    hMod = (pal::mod_t)::GetModuleHandleW(hostpolicyName);
    if (hMod != nullptr)
        return true;
...
#else // !TARGET_WINDOWS
    if (!pal::string_ends_with(mock_hostpolicy_value, pal::nativelib_ext))
        mock_hostpolicy_value.append(pal::nativelib_ext);

    hMod = (pal::mod_t)dlopen(mock_hostpolicy_value.c_str(), RTLD_LAZY);
    if (hMod == nullptr)
...
static bool try_load_coreclr(const pal::string_t& core_root, pal::mod_t& hMod)
{
...
{
    pal::string_t val = pal::getenv(var);
    if (val.empty())
        val = def ? W("1") : W("0");
...
static int run(const configuration& config)
{
    ...
    // Determine the maanged application's path.
    ...
    pal::string_t app_path_ni = app_path + W("NI");
    ...
    // CORE_LIBRARIES
    pal::string_t core_libs = pal::getenv(envvar::coreLibraries);
    ...
    pal::string_t core_root = !config.clr_path.empty()
        ? config.clr_path
        : pal::getenv(envvar::coreRoot);

    // If CORE_ROOT wasn't supplied use app_path, otherwise
    ...
    pal::string_t tpa_list = build_tpa(core_root, core_libs);

    {
        // Load hostpolicy if requested.
        pal::string_t mock_hostpolicy = pal::getenv(envvar::mockHostPolicy);
        if (!mock_hostpolicy.empty()
            && try_load_hostpolicy(mock_hostpolicy))
        {
            return -1;
        }
    }
    ...
    if (!try_load_coreclr(core_root, coreclr_mod))
    {
        return -1;
    }
    ...
    // Construct arguments
    //
    pal::string_utf8_t exe_path_utf8 = pal::convert_to_utf8(pal::get_exe_path());
    ...
    configuration& config)
{
    ...
    {
        bool is_option = argv[i][0] == W('-') || argv[i][0] == W('/');
